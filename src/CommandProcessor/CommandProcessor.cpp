...
CommandProcessor::CommandProcessor() { commands = std::vector<Command>(); }
...
bool CommandProcessor::getCommand(Command *_newCommand) {
  // reads a command from the console and saves it in the command list, returns
...
bool CommandProcessor::validate(Command &_command,
                                const std::string &_currentState) {
  ...
  // if the current state is in the list of valid states (i.e. the returned
  // iterator is not the end of the vector), then the command is valid
  return std::find(_command.validStates.begin(), _command.validStates.end(),
                   _currentState) != _command.validStates.end();
}
...
bool CommandProcessor::readCommand(Command *_newCommand) {
  ...
  // checks if the command is valid and sets the next state and valid states
  // accordingly
...
FileCommandProcessorAdapter::FileCommandProcessorAdapter(
    const FileCommandProcessorAdapter &_other)
    : CommandProcessor(_other) {
  // see FileLineReader.h for more information and warnings about its assignment
  // operator
