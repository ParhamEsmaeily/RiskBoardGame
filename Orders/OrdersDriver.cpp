...
void testOrdersList()
{
    ...
    //Deploy
    Deploy* deploy = new Deploy(p1, &*territories[0], 6);
    cout << "validation of:\n" << deploy << "\n" << deploy->validate() << endl;
    Deploy* deploy2 = new Deploy(p1, &*territories[1], 6);
    cout << "validation of:\n" << deploy << "\n" << deploy->validate() << endl;
    //Advance
    Advance* advance = new Advance(p1, p2, &*territories[0], &*territories[1], 6);
    cout << "validation of:\n" << advance << "\n" << advance->validate() << endl;
    Advance* advance2 = new Advance(p1, p2, &*territories[0], &*territories[7], 6);
    cout << "validation of:\n" << advance << "\n" << advance->validate() << endl;
    //Test advance and combat mechanics
    p2->setTerritoryUnits(&*territories[1], 20);
    p1->setTerritoryUnits(&*territories[0], 10);
    Advance* advance3 = new Advance(p1, p2, &*territories[0], &*territories[1], 6);
    cout << advance3;
    advance3->execute();
    ...
    Advance* advance4 = new Advance(p1, p2, &*territories[0], &*territories[1], 20);
    cout << advance;
    advance4->execute();
    ...
    Negotiate* nego = new Negotiate(p1, p2, negocard);
    Advance* advance5 = new Advance(p1, p2, &*territories[1], &*territories[4], 1);
    ...
    Blockade* blockade = new Blockade(p1, neutral, blocard, &*territories[1]);
