...
void testCommandProcessor(const std::string &_fileName)
{
    ...
    // if no file name is provided, then the command processor will read commands from the console
    if (_fileName.compare("") == 0)
    {
        CommandProcessor commandProcessor = CommandProcessor();

        std::cout << "Testing command processor from console input. Enter an unknown command to exit." << std::endl;

        // reads commands from the console until an invalid/unknown command is entered
        while (true)
        {
    ...
    else
    {
        ...
        FileCommandProcessorAdapter fileCommandProcessorAdapter = FileCommandProcessorAdapter(&fileLineReader);

        std::cout << "Testing command processor from file input (" << _fileName << ")... Enter the initial game state to start." << std::endl;
        ...
        while (true)
        {
            ...
            std::cout << "Valid command: {" << newCommand << "}"
                      << std::endl;
        }
    }
